...
- a `Connect` switch (master on/off; see below)
...
connect toggle, target selection, launch-at-login toggle, show control panel,
open log, and quit.

### Connect switch

The **Connect** switch (in the popover header, or the `Connect` item in the
right-click menu) is the master on/off:

- **On** — connect the iPad now and keep it connected: the app reconnects after
  wake/unlock/display changes. This is the normal mode.
- **Off** — disconnect the iPad now and leave it alone, so you can use it as a
  standalone iPad (read on it, AirPlay a movie, etc.). The app stays running but
  does not reconnect, the status reads `Off`, and the menu-bar icon dims. It also
  re-disconnects on launch if something reconnected the iPad while you were off.

Turn it back **On** to reconnect and resume automatic reconnection.
...
- retries one attempt at a time with exponential backoff (8s, then doubling);
  after a few consecutive failures it gives up and **suspends** auto-reconnect,
  because each failed connect leaves a macOS "Unable to Connect (-501)" dialog
  that never auto-dismisses. Only a genuine wake / unlock / manual reconnect /
  Connect toggle re-arms it — the noisy `screen parameters changed` event (which
  fires constantly, and is itself emitted by our own successful connect) will
  not restart a suspended cycle
- ignores `screen parameters changed` for a short window after a successful
  connect, so attaching the Sidecar display does not immediately trigger another
  attempt into the still-settling link (which returns -501)
- never shows its own duplicate alert
